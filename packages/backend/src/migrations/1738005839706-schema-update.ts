...
export class SchemaUpdate1738005839706 implements MigrationInterface {
    name = 'SchemaUpdate1738005839706'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "shipment" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "canisterShipmentId" bigint NOT NULL, "status" "public"."shipment_status_enum" NOT NULL DEFAULT 'PENDING', "value" numeric(10,2) NOT NULL, "price" numeric(10,2) NOT NULL, "size" text NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "trackingToken" character varying, "pickupWindowStart" TIMESTAMP WITH TIME ZONE, "pickupWindowEnd" TIMESTAMP WITH TIME ZONE, "deliveryWindowStart" TIMESTAMP WITH TIME ZONE, "deliveryWindowEnd" TIMESTAMP WITH TIME ZONE, "shipperPrincipal" character varying NOT NULL, "carrierPrincipal" character varying, "pickupAddressId" uuid, "deliveryAddressId" uuid, CONSTRAINT "UQ_578e971708483c63fc04c325db4" UNIQUE ("canisterShipmentId"), CONSTRAINT "REL_22798fce4976de82c80bce222e" UNIQUE ("pickupAddressId"), CONSTRAINT "REL_7b9090c09dccd175e0042711b7" UNIQUE ("deliveryAddressId"), CONSTRAINT "PK_f51f635db95c534ca206bf7a0a4" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`ALTER TABLE "route_delay" ADD CONSTRAINT "FK_506531caa74d474ed486a551c3e" FOREIGN KEY ("stopId") REFERENCES "route_stop"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
    }
